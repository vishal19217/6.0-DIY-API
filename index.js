...
// It is use to encode the body to json object automatically
app.use(express.json());

//1. GET a random joke
app.get("/random",(req,res)=>{
  let randomJoke = Math.floor(Math.random()*jokes.length);
  let result = jokes[randomJoke];
  console.log(result); 
  res.json(result);
});
...
app.get("/jokes/:id",(req,res)=>{
  ...
  console.log(params);
  let foundJoke = jokes.find((joke) => joke.id === params);
    res.json(foundJoke);
...
app.put("/jokes/:id",(req,res)=>{
  ...
  console.log(replacedJoke);
  res.json(replacedJoke);
});
//6. PATCH a joke
app.patch("/jokes/:id",(req,res)=>{
  let id = parseInt(req.params.id);
  let currentJokeIndex = jokes.findIndex((joke)=>{
    return joke.id === id;
  });
  let currentJoke = jokes.find((joke)=>{
    return joke.id === id;
  });
  ...
  jokes[currentJoke] = replacedJoke;
  res.json(replacedJoke);
  console.log(replacedJoke);
});
...
app.delete("/jokes:id",(req,res)=>{
  ...
  if(currentJokeIndex!=-1)
    jokes.splice(currentJokeIndex,1);
  else{
    res.status(404)
        .json({
          error: `id ${id} not found in array `
        })
  }
});
